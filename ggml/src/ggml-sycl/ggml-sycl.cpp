...
static int ggml_sycl_mul_mat_ffn_mmvq_fused(ggml_backend_sycl_context & ctx, ggml_cgraph * cgraph, int node_idx) {
    ...
    if (nup->op != GGML_OP_MUL_MAT || nup->type != GGML_TYPE_F32 ||
        nup->src[1] != act) {
        ++g_ffn_decline[4];
        return 0;
    }
    if (nglu->op != GGML_OP_GLU || nglu->src[0] != ngate ||
        nglu->src[1] != nup) {
        ++g_ffn_decline[5];
        return 0;
    }
    ...
    // this writes the outputs directly rather than the per-device row slices that
    // ggml_sycl_op_mul_mat() stitches back together, so it cannot serve split weights
    if (ggml_backend_buffer_is_sycl_split(wg->buffer) ||
    ...
    if (fused_glu) {
        ...
        if (ffn_diag >= 3 && (call_idx == 0 || call_idx == 1)) {
            ...
            if (ffn_diag == 5) {
                mul_mat_vec_pq2_0_batched_sycl_v13(wg->data, wu->data, src1_ddq,
                                                   (float *) ngate->data,
                                                   (float *) nup->data,
                                                   (int) ne00, (int) wg->ne[1],
                                                   (int) wu->ne[1], stream);
                ffn_glu_epilogue_sycl((const float *) ngate->data,
                                      (const float *) nup->data,
                                      (float *) nglu->data,
                                      (int) wg->ne[1], 1, stream);
            } else if (ffn_diag == 4) {
        ...
        if (ffn_diag == 2 && (call_idx == 0 || call_idx == 1 || call_idx == 2)) {
            ...
            mul_mat_vec_pq2_0_batched_sycl_v14d(wg->data, wu->data, src1_ddq,
                                                ...
                                                (int) ne00, (int) wu->ne[1],
                                                stream);
            const int probe_rows[6] = {0, 1, 100, 1000, 5000, 17307};
            float h[12] = {0};
            for (int pr = 0; pr < 6; ++pr) {
                (void) stream->memcpy(&h[2*pr], diag_alloc.get() + (size_t) probe_rows[pr] * 2, 2 * sizeof(float));
            }
            stream->wait();
            printf("[FFN_DIAG] v14d call=%ld:", call_idx);
            for (int pr = 0; pr < 6; ++pr) {
                printf(" r%d=(%.6g,%.6g)", probe_rows[pr], h[2*pr], h[2*pr+1]);
            }
            printf("\n");
            fflush(stdout);
            return 3;
...

    {
        ...
        if (ffn_diag13 && (call_idx == 0 || call_idx == 1 || call_idx == 2 || call_idx == 512)) {
            ...
            float hu13[6] = {0};
            for (int pr = 0; pr < 6; ++pr) {
                (void) stream->memcpy(&hg13[pr], (const float *) ngate->data + probe_rows13[pr], sizeof(float));
                (void) stream->memcpy(&hu13[pr], (const float *) nup->data + probe_rows13[pr], sizeof(float));
            }
            stream->wait();
            printf("[FFN_DIAG] v13 call=%ld:", call_idx);
            for (int pr = 0; pr < 6; ++pr) {
                printf(" r%d=(%.6g,%.6g)", probe_rows13[pr], hg13[pr], hu13[pr]);
            }
            printf("\n");
            fflush(stdout);
        }
